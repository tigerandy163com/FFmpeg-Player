...
static void avStreamFPSTimeBase(AVStream *st, CGFloat defaultTimeBase, CGFloat *pFPS, CGFloat *pTimeBase, AVCodecContext *codec) {
    ...
    if (codec->ticks_per_frame != 1) {
        LoggerStream(0, @"WARNING: st.codec.ticks_per_frame=%d", codec->ticks_per_frame);
...
- (kxMovieError)openVideoStream:(NSInteger)videoStream {
    ...
    AVStream *stream = _formatCtx->streams[videoStream];
    ...
    _sourceWidth = _videoCodecCtx->width;
    _sourceHeight = _videoCodecCtx->height;
    // determine fps
    _videoFrame->width = _sourceWidth;
    _videoFrame->height = _sourceHeight;
    _videoFrame->format = _videoCodecCtx->pix_fmt;
    AVStream *st = _formatCtx->streams[_videoStream];
    avStreamFPSTimeBase(st, 0.04, &_fps, &_videoTimeBase, _videoCodecCtx);
    ...
    LoggerVideo(1, @"video codec size: %d:%d fps: %.3f tb: %f",
            ...
            _videoTimeBase);
    // 分配视频帧
    _outputWidth = _outputWidth ?: _sourceWidth;
    _outputHeight = _outputHeight ?: _sourceHeight;
    _dst_frame = av_frame_alloc();
    _dst_frame->format = AV_PIX_FMT_RGB24;
    _dst_frame->width = _outputWidth;
    _dst_frame->height = _outputHeight;
...
- (kxMovieError)openAudioStream:(NSInteger)audioStream {
    ...
    if (!codecCtx)
        return AVERROR(ENOMEM);
    AVStream *stream = _formatCtx->streams[audioStream];
    avcodec_parameters_to_context(codecCtx, stream->codecpar);
    ...
    AVStream *st = _formatCtx->streams[_audioStream];
    avStreamFPSTimeBase(st, 0.025, 0, &_audioTimeBase, _audioCodecCtx);
...
- (kxMovieError)openSubtitleStream {
    kxMovieError errCode = kxMovieErrorStreamNotFound;
    _subtitleStream = -1;
    _subtitleStreams = collectStreams(_formatCtx, AVMEDIA_TYPE_SUBTITLE);
    for (NSNumber *n in _subtitleStreams) {
        
        errCode = [self openSubtitleStream:n.integerValue];
        if (errCode == kxMovieErrorNone)
            break;
    }
    return errCode;
...
- (kxMovieError)openSubtitleStream:(NSInteger)subtitleStream {
    ...
    if (!codecCtx)
        return AVERROR(ENOMEM);
    AVStream *stream = _formatCtx->streams[subtitleStream];
    avcodec_parameters_to_context(codecCtx, stream->codecpar);
...
- (BOOL)setupScaler {
    [self closeScaler];
    av_freep(_dst_frame->data);
    _pictureValid = av_image_alloc(_dst_frame->data, _dst_frame->linesize, _dst_frame->width, _dst_frame->height, _dst_frame->format, 1) > 0;
...
- (KxVideoFrame *)handleVideoFrame {
    ...
    if (_videoFrameFormat == KxVideoFrameFormatYUV) {
        ...
        rgbFrame.linesize = _dst_frame->linesize[0];
        rgbFrame.rgb = [NSData dataWithBytes:_dst_frame->data[0]
                                      length:rgbFrame.linesize * _videoCodecCtx->height];
        frame = rgbFrame;
        frame.width = _dst_frame->width;
        frame.height = _dst_frame->height;
    }
...
- (KxAudioFrame *)handleAudioFrame {
    ...
    if (_swrContext) {
        float sampleRatio = audioManager.samplingRate / _audioCodecCtx->sample_rate;
        float channelRatio = audioManager.numOutputChannels / _audioCodecCtx->channels;
        float ratio = sampleRatio * MAX(1, channelRatio);
        int samples = _audioFrame->nb_samples * ratio;
...
- (KxSubtitleFrame *)handleSubtitle:(AVSubtitle *)pSubtitle {
    ...
    frame.text = [ms copy];
    frame.position = ((CGFloat)pSubtitle->pts / AV_TIME_BASE + pSubtitle->start_display_time) * 1000;
    ...
    while (!finished) {
        ...
        if (packet.stream_index == _videoStream) {
            ...
            while (pktSize > 0) {
                ...
                BOOL gotframe = avcodec_send_packet(_videoCodecCtx, &packet) == 0;
                int len = 0;
                if (gotframe) {
...
//                                              _videoCodecCtx->height);
            ...
            while (pktSize > 0) {
                ...
                BOOL gotframe = avcodec_send_packet(_audioCodecCtx, &packet) == 0;
                int len = 0;
                if (gotframe) {
                    len = avcodec_receive_frame(_audioCodecCtx, _audioFrame);

                    if (len < 0) {
                        LoggerVideo(0, @"decode audio error, skip packet");
                        break;
                    }
                }
